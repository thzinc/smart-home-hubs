...
VIDEO_RECORDER_SYNC_ENABLED=${VIDEO_RECORDER_SYNC_ENABLED:-}
...
sync_repeatedly() {
    if [ -z "$VIDEO_RECORDER_SYNC_ENABLED" ]; then
        echo "VIDEO_RECORDER_SYNC_ENABLED is not set; skipping sync with off-site storage"
    else
